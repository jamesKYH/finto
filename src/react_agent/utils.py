...
async def load_mcp_config_json(filepath: str = "mcp_config.json") -> Dict[str, Any]:
    ...
    try:
        ...
        # Set default embedding vector name for Qdrant servers
        for server_name, server_config in config.get("mcpServers", {}).items():
            if server_config.get("command") == "mcp-server-qdrant":
                env = server_config.setdefault("env", {})
                if "QDRANT_VECTOR_NAME" not in env:
                    env["QDRANT_VECTOR_NAME"] = "all-MiniLM-L6-v2"
